    ...
    , mHP(hpComp)
{
    if (owner && hpComp)
    {
        ...
        mBackgroundSprite = new SpriteComponent(mHPBarActor, 199);
        mBackgroundSprite->SetTexture(owner->GetGame()->GetTexture("Assets/HPBarBack.png"));

        mBarSprite = new SpriteComponent(mHPBarActor, 200);
        mBarSprite->SetTexture(owner->GetGame()->GetTexture("Assets/HPBar1.png"));
    }
}
